...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model):
        ...
        artifact = wandb.Artifact("trained_pawnn", type="model")
...
hparams = {
    ...
    "weight_decay": 1e-8,
    "input_size": 7448,
    "hidden_sizes_per_bus": [64, 32, 32, 32, 16, 16, 8, 4],  
...
with wandb.init(project="PAWNN Training AUS P1R complete base", config=hparams):
    config = wandb.config
    ...
    # Log weights and gradients as histograms at epoch level
    def log_model_parameters(model, epoch):
        for name, param in model.named_parameters():
            ...
            # Log gradients as a histogram if gradients exist
    ...
    def train_model(model, train_loader, val_loader, loss_func, epochs=10, name="default"):
        ...
        """
        wandb.watch(model, loss_func, log="all", log_freq=25)

        optimizer = torch.optim.Adam(model.parameters(), lr=hparams["learning_rate"], weight_decay=hparams["weight_decay"])
        #optimizer = torch.optim.SGD(model.parameters(), lr=hparams['learning_rate'], weight_decay=hparams['weight_decay'], )
        #scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=int(epochs * len(train_loader) / 5), gamma=hparams["gamma"])
        scheduler = torch.optim.lr_scheduler.CyclicLR(
            optimizer,
            base_lr=hparams['learning_rate'],         # Matches initial learning rate
        ...
        early_stopping = EarlyStopping(patience=10, min_delta=1e-4, path=name + "trained_pawnn.pth")
        ...
        for epoch in range(epochs):
            ...
            with torch.no_grad():
                ...
                for batch in val_loader:
                    ...
                    pred = model(features)
                    # Clamp the angle predictions to the valid range
                    pred[:, angle_target_indices] = torch.clamp(pred[:, angle_target_indices], min=min_angle, max=max_angle)
    ...
    train_model(model, labled_train_loader, labled_val_loader, loss_func, epochs=epochs, name="complete_base")
    ...
    test_loss, test_mape, test_r2 = test_model(
        ...
        target_columns=target_columns,
        device=device,
        results_path=data_path + "test_results_complete_base.xlsx"
    )
